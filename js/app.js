...
EX.App = (function () {
  ...
  function prepararInicio() {
    ...
    try {
      var previo = JSON.parse(localStorage.getItem('examen-jueces-alumno') || 'null');
      if (previo) { nombre.value = previo.nombre || ''; apellido.value = previo.apellido || ''; }
    } catch (e) {}

    etiquetaRegistro.textContent = EX.Sheets.disponible()
      ? 'Los resultados se envían a la hoja RC8'
      : 'Sin hoja configurada: se guardan en este dispositivo';
    ...
    function comenzar() {
      ...
      EX.Audio.iniciar();
      try { localStorage.setItem('examen-jueces-alumno', JSON.stringify({ nombre: n, apellido: a })); } catch (e) {}

      registro.className = 'registro esta-enviando';
      etiquetaRegistro.textContent = 'Registrando en la hoja RC8…';

      EX.Sheets.registrarInicio(n, a).then(function (r) {
        if (r.ok) {
          registro.className = 'registro es-ok';
          etiquetaRegistro.textContent = 'Registrado en la hoja RC8';
        } else if (r.motivo === 'sin-configurar') {
          registro.className = 'registro';
          etiquetaRegistro.textContent = 'Guardando en este dispositivo';
        } else {
          registro.className = 'registro es-error';
          etiquetaRegistro.textContent = 'La hoja no respondió; se guarda aquí';
        }
  ...
  function mostrarResultado(datos) {
    ...
    contenedor.appendChild(U.crear('span', {
      clase: 'eyebrow',
      texto: datos.alumno.nombre + ' ' + datos.alumno.apellido + ' · Capítulo 8'
    }));
    ...
    var estadoEnvio = U.crear('span', { clase: 'registro esta-enviando' }, [
      ...
      U.crear('span', { texto: 'Enviando el resultado a la hoja RC8…' })
    ]);
    ...
    contenedor.appendChild(U.crear('div', { clase: 'acciones' }, [
      ...
      U.crear('a', {
        clase: 'boton boton--linea', href: CFG.hojaCalculo.url, target: '_blank', rel: 'noopener'
      }, ['Abrir la hoja RC8'])
    ]));
    ...
    EX.Sheets.registrarFinal({
      ...
      U.vaciar(estadoEnvio);
      estadoEnvio.className = 'registro ' + (r.ok ? 'es-ok' : 'es-error');
      estadoEnvio.appendChild(U.crear('span', { clase: 'registro__luz' }));
      estadoEnvio.appendChild(U.crear('span', {
        texto: r.ok
          ? 'Resultado guardado en la hoja RC8'
          : (r.motivo === 'sin-configurar'
            ? 'Guardado en este dispositivo (hoja sin configurar)'
            : 'No se pudo enviar a la hoja; queda guardado aquí')
      }));
    });
  }
  ...
  function descargarRespaldo() {
    ...
    U.descargarCSV('resultados-RC8.csv', filas);
    U.brindis('CSV descargado: ábrelo y pégalo en la hoja RC8 si hace falta.');
  }
